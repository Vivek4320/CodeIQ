...
export default function TemplateSelector({ language, onSelect, onClose }: TemplateSelectorProps) {
  ...
  const filtered = templates.filter((t) => t.language === selectedLang && (!search || t.name.toLowerCase().includes(search.toLowerCase()) || t.description.toLowerCase().includes(search.toLowerCase())));
  const languages = [...new Set(templates.map((t) => t.language))];
  const handleSelect = (template: Template) => { onSelect(getTemplateCode(template), template.language); onClose(); };
